	...
	while(!done)
	{
		...
		for(vector<struct rule> :: iterator it = result.begin();it!=result.end();it++)
		{
			...
			if(dot_pos!=it->right.length()-1)
			{
				...
				{
					//found an item of the form A->alpha.Bbeta
					//so we'd have to add B->.gamma
...
bool areSame(vector<struct rule> I1, vector<struct rule> I2)
{
    ...
    {
        if(I1[i].left!=I2[i].left||(I1[i].right.compare(I2[i].right)!=0))
        return false;
    }
    return true;
}
    ...
    for(vector<vector<struct rule> >::iterator it = canonical_LR_items.begin();it!=canonical_LR_items.end();it++)
    {
        ...
        for(vector<struct rule>::iterator vt = I.begin();vt!=I.end();vt++)
        {
            ...
            else
            {
                ...
                set<char> follow_of_A = follow(vt->left, fol, fir, r, n);
                struct action_entry entry;
                ...
                for(set<char>::iterator ft = follow_of_A.begin();ft!=follow_of_A.end();ft++)
                {
                    if(ACTION.find(make_pair(item_to_number(I),*ft))==ACTION.end())
                        ACTION[make_pair(item_to_number(I), *ft)] = entry;
                    else
                        {
                            map<pair<int, char>, struct action_entry>::iterator actt = ACTION.find(make_pair(item_to_number(I),*ft));
                            cout<<"Multiple entries for "<<item_to_number(I)<<", "<<*ft<<": \n";
                            cout<<"Existing entry: ";
